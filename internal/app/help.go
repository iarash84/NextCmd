...
func printGeneralHelp(writer io.Writer, engine *completion.Engine) {
	...
	fmt.Fprintln(writer, "  Tab/Right     Accept a suggestion; Tab then moves through placeholders")
	...
	fmt.Fprintln(writer, "  Enter         Execute the current command")
	fmt.Fprintln(writer, "  Shift+Enter   Insert a new line without executing")
	...
	fmt.Fprintln(writer, "Safety:")
	fmt.Fprintln(writer, "  Recognized dangerous commands require confirmation (default: no)")
	fmt.Fprintln(writer, "  Append --yes for deliberate non-interactive approval")
	...
	fmt.Fprintln(writer, "  :del <path>           Move to trash; use --permanent [--yes] carefully")
